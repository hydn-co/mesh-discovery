...
func (c *AccountEntityCollector) Start(ctx context.Context) error {
	...
	// Emit one consolidated AccountExtension per account. Oversized payloads are
	// sharded transparently by mesh-core's storage layer (hydn-co/mesh-core#271);
	// the connector just emits the whole logical entity.
